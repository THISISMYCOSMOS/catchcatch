...
일반 동일상품 검색에서도 URL·검색 출처·상품 동일성 검사를 통과한 `MUSINSA_BEAUTY`
`AVAILABLE` 후보만, 최대 2 MB·기본 3초 제한 안에서 해당 판매 페이지 메타데이터를 다시
확인합니다. 현재 공개 판매가가 있으면 AI가 반환한 정상가·판매가를 직접 값으로 교체하고
`CONTENT_VERIFIED`로 승격합니다. 가격이 없거나 직접 검증이 실패하면 AI 가격·할인·배송비
금액을 모두 비워 `URL_VERIFIED` 상태에 남기며, 직접 페이지가 품절이라고 하면 후보를
`NOT_AVAILABLE`로 강등합니다. 올리브영·쿠팡·지그재그·브랜드 공식몰에는 이 직접 검증기를
임의로 적용하지 않습니다.
