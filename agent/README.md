...
지그재그는 Luna 웹 검색으로 후보 URL을 찾은 뒤 공개 상품 페이지의 서버 렌더링 데이터로
상품명·판매 가능 여부·정상가·현재 표시가·공개 쿠폰·배송비를 다시 확인합니다.

일반 동일상품 검색에서도 URL·검색 출처·상품 동일성 검사를 통과한 `MUSINSA_BEAUTY`와
`ZIGZAG`의 `AVAILABLE` 후보만, 최대 2 MB·기본 3초 제한 안에서 해당 판매 페이지를 다시
확인합니다. 현재 공개 판매가가 있으면 검색 결과의 가격을 직접 값으로 교체하고
`CONTENT_VERIFIED`로 승격합니다. 가격이 없거나 직접 검증이 실패하면 검색에서 얻은
가격·할인·배송비 금액을 모두 비워 `URL_VERIFIED` 상태에 남기며, 직접 페이지가 품절이라고
하면 후보를 `NOT_AVAILABLE`로 강등합니다. 지그재그는 URL 상품번호와 페이지 상품번호,
검색 상품명과 페이지 상품명도 함께 대조합니다. 올리브영·쿠팡·브랜드 공식몰에는 이 직접
검증기를 임의로 적용하지 않습니다.
