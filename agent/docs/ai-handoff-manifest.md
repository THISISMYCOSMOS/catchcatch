...
`similar-product-search` 프롬프트·스키마(유사상품 검색)는 서비스·컨트롤러·`app.module.ts` 등록이 끝내 없어 실행 경로가 없는 미연결 코드였으므로 삭제했다. 관련 설계 논의는 `docs/ai-prompts.md`, `docs/ai-schema-contracts.md` 등에 이력으로만 남아 있다.
...
현재 테스트는 프롬프트 핵심 금지사항, 입력 경계, 다섯 판매처의 정확한 반환, 출처 도메인 검증, 빈 검색 결과 허용, AI 응답의 구조 검증 및 재시도를 확인한다.
...
- 유사상품 검색은 프롬프트·스키마 설계 단계에서 실행 서비스로 이어지지 않아 미연결 코드로 삭제했다(§2 참고). 다시 만들려면 새 설계로 시작해야 한다.
