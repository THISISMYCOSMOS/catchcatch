...
- 버전: `catchcatch-product-search-v7`
- 실행 파일: `src/product-search/product-search.prompt.ts`
- 검색 대상: 올리브영, 무신사 뷰티, 쿠팡, 지그재그, 백엔드 등록 브랜드 공식몰
...
외부 요청에는 공식몰 도메인을 받지 않는다. 서비스가 식별된 브랜드명으로 공식몰을 별도 `web_search`하고, 후보 도메인과 일치하는 근거 URL이 제공자 반환 출처 목록에 실제로 존재하며 규칙 기반 게이트까지 통과한 경우에만 검색 프롬프트의 `registered_brand_official_domain`에 채워 넣는다. `brand_id` 입력은 하위 호환을 위해 남아 있지만 이 과정에 쓰이지 않는다.
