...
외부 입력에서 공식몰 도메인을 제거했다. (T5 갱신) 사람이 큐레이션하는 `brand_id` 레지스트리(`BRAND_OFFICIAL_DOMAINS_JSON`)는 확장성이 없어 폐기했고, 대신 식별된 브랜드명으로부터 후보 도메인을 발견한 뒤 규칙 기반 게이트(마켓플레이스·고정 판매처 도메인 차단, `.kr` 아닌 도메인은 차단 대신 경고)를 통과한 도메인만 신뢰한다. `brand_id`는 입력에는 남지만 이 조회에 쓰이지 않는다.

상태: 코드와 테스트 반영 완료. 다만 게이트는 "확실히 틀린 후보"만 걸러내며, 통과한 도메인이 실제로 그 브랜드의 것임을 확인하지는 않는다. 남는 위험은 아래 "잔여 위험: 브랜드 공식몰 도메인 발견"에 정리한다.

### 잔여 위험: 브랜드 공식몰 도메인 발견

게이트(`gateBrandOfficialDomainCandidate`)가 하는 일은 **후보 도메인의 문자열 형태를 보고 명백히 틀린 부류를 거부**하는 것뿐이다. 다음은 게이트가 확인하지 **않는** 것이다.

- 도메인이 실재하는지 (DNS·HTTP 조회를 하지 않는다)
- 도메인 소유자가 누구인지
- 그 도메인이 해당 브랜드와 실제로 어떤 관계인지
- 그 도메인에 문제의 상품이 실제로 있는지

따라서 게이트를 통과했다는 사실은 "이 도메인은 마켓플레이스가 아니고 우리 고정 판매처도 아니며 HTTPS 호스트명 형태다"라는 뜻 이상이 아니다. 아래는 그럼에도 `BRAND_OFFICIAL`로 승격될 수 있는 경우다. 승격의 실질적 영향은 공통이다. 해당 도메인이 `web_search`의 `allowed_domains`에 들어가고, 그 도메인에서 나온 오퍼가 "브랜드 공식몰 가격"으로 비교표·최저가·추천 판매처 계산에 들어간다.
...
경고 자체는 강화했다. 이제 발견된 도메인을 쓰는 모든 요청에 "이 도메인은 모델이 제안했고 규칙 검사만 통과했으며 브랜드가 운영한다는 확인은 없다"는 경고가 무조건 붙는다(`buildBrandOfficialDomainWarnings`, 캐시 히트에도 동일). 다만 이것은 신호를 전달할 뿐이고, **경고를 근거로 해당 오퍼를 비교에서 제외하거나 신뢰도를 낮추도록 강제하는 규칙은 여전히 파이프라인에 없다.** 그 처리는 Core 쪽 결정이며 Agent 범위 밖이다.
...
- 브랜드명 자체가 상품 페이지에서 AI가 추출한 값, 즉 신뢰 경계 밖에서 온 문자열이라는 점은 그대로다. 발견 호출이 `web_search`를 쓰지 않는다는 사실은 이 입력 경로까지 막아 주지 않는다.
...
| 무조건 경고 | 발견 도메인을 쓰면 항상 "미검증" 경고를 응답에 첨부(캐시 히트 포함) |
...
- 승격 전 도메인–브랜드 대조 확인 단계(실재 확인, 상호·사업자 정보 확인 등). ASCII 타이포스쿼트를 실제로 막으려면 이것이 필요하다
