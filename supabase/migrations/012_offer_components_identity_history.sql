alter table public.products
  ...
  add column shade_or_scent text,
  add column version_or_renewal text;

create table public.seller_offer_components (
  ...
  constraint seller_offer_components_component_type_chk
  ...
  constraint seller_offer_components_capacity_unit_chk
    check (capacity_unit is null or capacity_unit in ('ML', 'G')),
  constraint seller_offer_components_capacity_value_chk
    check (capacity_value is null or capacity_value > 0),
  constraint seller_offer_components_quantity_chk
    check (quantity is null or quantity > 0)
);

create index seller_offer_components_seller_offer_id_idx
  on public.seller_offer_components(seller_offer_id);
...
alter table public.price_history
  ...
  add column is_sale_observation boolean not null default false,
  add column observation_key text;

create unique index price_history_observation_unique
  on public.price_history(observation_key)
...
begin
  ...
  loop
    ...
    values (
      ...
      nullif(price_history_entry->>'listed_price', '')::numeric,
      nullif(price_history_entry->>'listed_sale_price', '')::numeric,
      coalesce((price_history_entry->>'is_sale_observation')::boolean, false),
      nullif(price_history_entry->>'observation_key', ''),
